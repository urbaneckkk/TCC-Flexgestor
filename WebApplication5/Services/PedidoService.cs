...
namespace WebApplication5.Services
{
    public class PedidoService
    {
        ...
        public int Criar(PedidoCriarDto dto, int idEmpresa)
        {
            dto.Pedido.IdEmpresa = idEmpresa;
            dto.Pedido.DthCriacao = DateTime.Now;
            dto.Pedido.Status = "PENDENTE";

            // Recalcula totais no servidor (não confia só no front)
            dto.Pedido.ValorProdutos = dto.Itens.Sum(i => i.ValorTotal);
            dto.Pedido.ValorTotal = dto.Pedido.ValorProdutos + dto.Pedido.ValorFrete - dto.Pedido.Desconto;
